...
-   `src/types/database.ts` --- TypeScript interfaces for the MongoDB document structure. Note that `Projects` and `Experiences` use a `briefDescription` field for the frontend display, while `detailedDescription` is reserved for other outputs (like AI CV generation).
